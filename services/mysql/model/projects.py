...
class Projects(SQLModel, table=True):
    ...
    event_type: Optional[ProjectEventTypes] = Field(
        default=None, sa_column=Column(Enum(ProjectEventTypes), nullable=True)
    )
    venue: str = Field(sa_column=Column(String(255), nullable=False))
    marketing_id: Optional[int] = Field(
    ...
    state_id: Optional[int] = Field(
        default=None, sa_column=Column(BIGINT(unsigned=True), ForeignKey("states.id"))
    )
    city_id: Optional[int] = Field(
        default=None, sa_column=Column(BIGINT(unsigned=True), ForeignKey("cities.id"))
    )
    ...
    showreels_status: Optional[int] = Field(
        default=None, sa_column=Column(SmallInteger, nullable=True)
    )
    ...
    project_classes: Optional["ProjectClasses"] = Relationship(
