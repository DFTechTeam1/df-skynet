...
class RoleHasPermissions(SQLModel, table=True):
    ...
    role_id: int = Field(sa_column=Column(BIGINT(unsigned=True), ForeignKey("roles.id"), primary_key=True))

    roles: Optional["Roles"] = Relationship(back_populates="role_has_permissions") # type: ignore
    permissions: Optional["Permissions"] = Relationship(back_populates="role_has_permissions") # type: ignore