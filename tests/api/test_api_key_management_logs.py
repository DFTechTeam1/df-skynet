...
from apps.controller.api_key_management import mask_secret, openrouter_logs_cache_key
...
from tests.helpers import OPENROUTER_LOGS_CACHE_PATTERN, clear_openrouter_logs
...
async def test_request_headers_are_never_exposed(authed_client, db_session):
    """200 OK; request_headers (carries the OpenRouter credential) is stripped, the rest is kept."""
    await clear_openrouter_logs(db_session)
...
async def test_response_body_key_and_hash_are_masked(authed_client, db_session):
    """200 OK; the plaintext OpenRouter key and the 64-hex key hash are never returned in full."""
    await clear_openrouter_logs(db_session)
    real_key = "sk-or-v1-" + "d" * 56
    real_hash = "2c5b87725d2972bd91d8a5feccdeb809d0b8e628c1250102da0beecd082dec02"
    DfEngineOpenrouterLogsFactory.create(
        method="POST",
    ...
    assert row["response_body"]["key"] == "sk-or-v1********dddd"
    assert row["response_body"]["data"]["hash"] == mask_secret(real_hash)
    assert row["response_headers"]["location"] == f"/api/v1/keys/{mask_secret(real_hash)}"
...
async def test_hash_in_request_url_is_masked(authed_client, db_session):
    ...
    DfEngineOpenrouterLogsFactory.create(
        method="DELETE",
        endpoint=f"https://openrouter.ai/api/v1/keys/{real_hash}",
    )

    row = (await authed_client.call("GET", URL)).json()["data"]["paginated"][0]
    assert real_hash not in row["endpoint"]
    assert row["endpoint"] == f"https://openrouter.ai/api/v1/keys/{mask_secret(real_hash)}"
...
async def test_rejects_out_of_range_params(authed_client, db_session):
    """422 when page < 1 or itemsPerPage over the cap, same bounds as /setting/logs."""
    await clear_openrouter_logs(db_session)
...
async def test_response_is_cached_with_a_ttl(authed_client, db_session):
    ...
    resp = await authed_client.call("GET", URL)
    assert resp.status_code == 200
    key = openrouter_logs_cache_key(1, 50)
