...
from services.mysql.factory import DfEngineExternalApiCallsFactory
from services.api_key_management import ApiKeyManagement
from services.redis import client as redis_client, CacheKeys
from utils import local_time
from tests.helpers import clear_external_api_call_logs
...
async def test_returns_logs_newest_first(authed_client, db_session):
    ...
    now = local_time()
    DfEngineExternalApiCallsFactory.create(endpoint="/keys", method="POST", created_at=now - timedelta(minutes=5))
    DfEngineExternalApiCallsFactory.create(endpoint="/keys/abc", method="DELETE", created_at=now)

    body = (await authed_client.call("GET", URL, params={"source": "openrouter"})).json()["data"]
...
async def test_request_headers_are_never_exposed(authed_client, db_session):
    ...
    await clear_external_api_call_logs(db_session)
    DfEngineExternalApiCallsFactory.create(
        name="Zola",
        method="POST",
        endpoint="/keys",
    ...
    row = (await authed_client.call("GET", URL, params={"source": "openrouter"})).json()["data"]["paginated"][0]
...
async def test_hash_in_request_url_is_masked(authed_client, db_session):
    ...
    await clear_external_api_call_logs(db_session)
    real_hash = "b" * 64
    DfEngineExternalApiCallsFactory.create(method="DELETE", endpoint=f"https://openrouter.ai/api/v1/keys/{real_hash}")
...
async def test_created_at_is_human_friendly(authed_client, db_session):
    ...
    await clear_external_api_call_logs(db_session)
    DfEngineExternalApiCallsFactory.create(created_at=datetime(2026, 1, 5, 9, 30))
...
async def test_pagination(authed_client, db_session):
    ...
    now = local_time()
    for i in range(5):
        DfEngineExternalApiCallsFactory.create(endpoint=f"/keys/{i}", created_at=now - timedelta(minutes=i))

    page_one = (
        await authed_client.call("GET", URL, params={"source": "openrouter", "itemsPerPage": 2, "page": 1})
    ).json()["data"]
    assert page_one["totalData"] == 5
    assert len(page_one["paginated"]) == 2

    page_three = (
        await authed_client.call("GET", URL, params={"source": "openrouter", "itemsPerPage": 2, "page": 3})
    ).json()["data"]
...
async def test_rejects_out_of_range_params(authed_client, db_session):
    ...
    await clear_external_api_call_logs(db_session)
    assert (
        await authed_client.call("GET", URL, params={"source": "openrouter", "page": 0}, raise_for_status=False)
    ).status_code == 422
    assert (
        await authed_client.call(
            "GET", URL, params={"source": "openrouter", "itemsPerPage": 999}, raise_for_status=False
        )
    ).status_code == 422


@pytest.mark.asyncio
async def test_rejects_invalid_source(authed_client, db_session):
    """422 when `source` isn't one of the known LogSourceEnum values."""
    resp = await authed_client.call("GET", URL, params={"source": "unknown"}, raise_for_status=False)
    assert resp.status_code == 422


@pytest.mark.asyncio
async def test_response_is_cached_with_a_ttl(authed_client, db_session):
    """200 OK; a GET populates the logs cache (keyed by source + page + page size) with an expiry."""
    await clear_external_api_call_logs(db_session)
    DfEngineExternalApiCallsFactory.create(endpoint="/keys")
    redis = redis_client()

    assert (await authed_client.call("GET", URL, params={"source": "openrouter"})).status_code == 200
    key = cache_key.api_logs("openrouter", 1, 50)
...
async def test_second_fetch_is_served_from_cache(authed_client, db_session):
    ...
    first = (await authed_client.call("GET", URL, params={"source": "openrouter"})).json()["data"]
    assert first["totalData"] == 1

    DfEngineExternalApiCallsFactory.create(endpoint="/keys/xyz", method="DELETE")
    still_cached = (await authed_client.call("GET", URL, params={"source": "openrouter"})).json()["data"]
    assert still_cached["totalData"] == 1  # stale on purpose — served from cache

    await clear_external_api_call_logs(db_session)
    DfEngineExternalApiCallsFactory.create(endpoint="/keys")
    DfEngineExternalApiCallsFactory.create(endpoint="/keys/xyz", method="DELETE")
    fresh = (await authed_client.call("GET", URL, params={"source": "openrouter"})).json()["data"]
    assert fresh["totalData"] == 2


@pytest.mark.asyncio
async def test_returns_udin_logs(authed_client, db_session):
    """200 OK; ?source=udin returns only udin-typed rows, formatted without OpenRouter masking."""
    await clear_external_api_call_logs(db_session)
    DfEngineExternalApiCallsFactory.create(type="udin", endpoint="/storage/folders", method="POST")
    DfEngineExternalApiCallsFactory.create(type="openrouter", endpoint="/keys")

    body = (await authed_client.call("GET", URL, params={"source": "udin"})).json()["data"]
    assert body["totalData"] == 1
    assert body["paginated"][0]["endpoint"] == "/storage/folders"


@pytest.mark.asyncio
async def test_requires_auth(client):
    """401 when the request carries no bearer token."""
    resp = await client.call("GET", URL, params={"source": "openrouter"}, raise_for_status=False)
