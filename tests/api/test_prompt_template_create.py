...
async def test_create_success_returns_full_refreshed_list(authed_client):
    ...
    resp = await authed_client.call(
        "POST",
        URL,
        json={"name": name, "prompt": "a prompt", "description": "a desc", "is_active": True},
    )
...
@pytest.mark.parametrize(
    ...
    ids=["empty_name", "oversized_name", "empty_prompt", "oversized_prompt", "empty_description"],
)
...
async def test_create_duplicate_name_conflict(authed_client, db_session, user_id):
    ...
    resp = await authed_client.call(
        "POST", URL, json={"name": existing.name, "prompt": "a prompt"}, raise_for_status=False
    )
...
async def test_requires_auth(client):
    resp = await client.call(
        "POST", URL, json={"name": f"Unauth {uuid4().hex[:8]}", "prompt": "a prompt"}, raise_for_status=False
    )
